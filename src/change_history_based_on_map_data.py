...
def main():
    ...
    for file in Path(MAP_DATA_STATE_REGIONS_FOLDER).glob("*.txt"):
        state_map_data_file = file.as_posix()
        print(f"Stripping quotes from provinces {state_map_data_file}")
        strip_quotes_from_provinces(state_map_data_file, state_map_data_file)
    ...
    generate_province_terrains(True)
    ...
    print("Adding locators to 00_states_file.txt")
    move_generated_file = ""
    while move_generated_file != "y" and move_generated_file != "n":
        ...
        move_generated_file = (
            input(
                f"Move the locator files generated by the Map Editor to {MAP_OBJECT_DATA}? (y/n) (If in doubt type n): "
            )
            .lower()
            .strip()
        )
