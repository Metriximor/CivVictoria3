...
for file in Path("common/goods").glob("*.txt"):
    output_file_path = Path(
        f"common/modifier_types/building_{file.stem}_modifier_types.txt"
    )
    output_file_path.parent.mkdir(exist_ok=True, parents=True)
    with open(file, encoding=UTF_8_BOM) as goods_file, open(
        output_file_path, "w+", encoding=UTF_8_BOM
    ) as output_file:
...
"""
    ...
    for language in languages:
        localization_file_path = Path(
            f"localization/{language}/civ_modifiers_l_{language}.yml"
        )
        localization_file_path.parent.mkdir(exist_ok=True, parents=True)
        with open(
            localization_file_path, "w+", encoding=UTF_8_BOM
...
"""
            ...
            for good in generated_goods:
                ...
                localization_file.write(
                    f""" modifier_building_input_{good}_add: "@{good}! ${good}$ input per level"
