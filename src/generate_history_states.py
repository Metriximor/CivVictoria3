...
def create_pops(
    country_abrev: str, state_name: str, state_details: dict, country_data: dict
):
    result = ""
    pops = {}
    ...
    if state_name in state_details and "pops" in state_details[state_name]:
        ...
        pops = {}
        total_ratios: int = sum(
            [int(pop["ratio"]) for pop in state_details[state_name]["pops"]]
        )
...
def main():
    ...
    split_states: dict = json.loads(load_file_into_string(SPLIT_STATES_INPUT))
    state_details = yaml.load(load_file_into_string(STATES_DATA_INPUT))
    country_data = paradox.load(COUNTRY_DEFINITIONS)
    states_data = {
        state: MapDataState.parse_obj(inner_state_data)
        for state, inner_state_data in paradox.load(STATES_MAP_DATA).items()
    }

    # Prepare the data
    states = {
        state: country
        for country, state_names in states.items()
        for state in state_names
    }
    capital_states = {
        country_data[country]["capital"]
        if "capital" in country_data[country]
        else print(f"{country} has no defined capital in {COUNTRY_DEFINITIONS}")
    ...
    for state_name, state_data in states_data.items():
        ...
        if state_name in split_states:
            ...
            raise Exception(f"{state_name} is not defined in {STATES_INPUT}")
        ...
        for country_abbreviation in countries:
            ...
            pops += create_pops(
                country_abbreviation, state_name, state_details, country_data
            )
            pops += "    }\n"
        pops += "}\n"
            ...
            if state_name in capital_states:
                gov_admin = CreateBuilding(
                    ...
                    activate_production_methods={
                        "pm_simple_organization_government_administration"
                    },
                )
